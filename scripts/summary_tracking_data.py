...
def write_summary_data(filename, columns, units, maus_data):
    fout = open(filename, "w")
    header = zip(columns, units)
    for col, unit in header:
        print >> fout, col, "["+unit+"]",
    print >> fout
    for a_dict in maus_data:
        for col in columns:
...
def main():
    ...
    zgoubi_data = load_summary_data("zgoubi_summary_tracking_data.dat", columns, units)
    closed_orbits = load_json_file("closed_orbits_all.ref")
    cell_tunes = load_json_file("cell_tunes_nturns=10.1_stepsize=10.0_all.ref")
    ring_tunes = load_json_file("ring_tunes_nturns=100.1_stepsize=10.0_all.ref")
    maus_data = make_summary_data(closed_orbits, cell_tunes, ring_tunes)
    for a_maus in maus_data:
        for a_zgoubi in zgoubi_data:
            if abs(a_zgoubi["kinetic_energy"] - a_maus["kinetic_energy"]) < 0.1:
                print "zgoubi", [str(round(a_zgoubi[col], 3)).rjust(8) for col in columns]
                print "maus  ", [str(round(a_maus[col], 3)).rjust(8) for col in columns]
                print
    write_summary_data("maus_summary_data.dat", columns, units, maus_data)
